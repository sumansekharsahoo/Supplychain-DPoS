...
#Some instructions and assumptions made
...
#initializing objects for class Register(), Order(), Blockchain()
reg=Register()
ord= Order()
blk=Blockchain()
#unique OrderID is generated by incrementing this on every successful order placed
orderCnt=5001
#initializes genesis block by accepting current timestamp
...
"""
    ...
    #Register
        ...
        while True:
            ...
            #Applying .upper() so that 'c' is also valid
            if utype.upper()=='C':
        ...
        #Since a block in our blockchain contains 3 transacions, if there are <3 pending tranactions to be verified, not allowed to create block
        ...
        #printing the blockheader using printBlock() function
        if ch=='1':
            for i in blk.chain:
                blk.printBlock(i)
        #printing all those transactions that are verified and added into blockchain
        elif ch=='2':
            if len(blk.verifiedTransac)==0:
                ...
                for i in blk.verifiedTransac:
                    ...
                    for j in i:
                        print(j)
                    print("}\n")
        #printing all those transactions that are yet to be added into blockchain
        elif ch=='3':
        ...
        #cant buy unless there is atleast 1 distb and 1 manufacturer
        if len(reg.manufId)==0:
            ...
            if len(reg.distbId)==0:
                ...
                if buyer_id not in reg.consumerId:
                    ...
                    if prid not in range(1,13):
                        ...
                        #HERE WE IMPLEMENT THE 5th CONDITION MENTIONED IN PROJECT DESCRIPTION
                        #unless the distributor is not busy delivering a order, we wont allow him to take another delivery
                        # avDistb contains list of all available distb 
                        avDistb=[]
        ...
        if len(reg.manufId)==0:
            ...
            if len(reg.distbId)==0:
                ...
                if did not in ord.distbBusy:
                    ...
                    #on choosing yes, the product will be delivered to customer
                    #and the transaction will be added to pendingTransac of blockchain.py 
...
"""
                    #qrcode will contain the above text message and will be saved in name: qr{order_id}.png in the same directory
...
"""
        ...
        if len(ord.distbBusy)==0:
            ...
            #using printIncompleteOrders() function
            print("\nVIEW PENDING DELIVERY LIST\n")
            ord.printIncompleteOrders()

    elif choice=='8':
        #HERE WE IMPLEMENT THE 6th FEATURE (Resolving conflict between CLIENT and DISTRIBUTOR and PENALIZING THE LIAR)
        ...
        if cid not in reg.consumerId:
            ...
            if len(corders)==0:
                ...
                if ordid not in corders:
                    ...
                    #DISTRIBUTOR must agree to the claim that he delivered or not
                    distbcomp=input(f"DistributorID {ord.orders[ordid][2]}, do you claim to have delivered the product? (Y/N): ")
                    #if he accepts he didn't deliver, No issues
                    if distbcomp.upper()=='N':
                        print("Distributor agrees that he didn't deliver the product")
                    #if he agrees that he delivered, he accepts to enter the LIE DETECTING CHALLENGE    
